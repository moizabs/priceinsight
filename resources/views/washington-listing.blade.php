...
<div class="card-body card-block">
                    <div class="row mb-3">
                        ...
                        <div class="col-md-8 text-right">
                            <button name="add" class="btn-primary btn-sm addBtn"
                                style="margin-right: -58px; height: 37px; ">
                                Add Records
                            </button>
                        </div>
                ...
                <div class="pagination-container" id="paginationContainer"></div></br>
          </div>
                </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="text" placeholder="Origin" class="form-control Origin_ZipCode typeahead"
                                id="origin" required>
                        </div>
                        <div class="form-group">
                            <label>Destination Location</label>
                            <input type="text" placeholder="Destination" class="form-control Dest_ZipCode typeahead"
                                id="destination" required>
                            {{-- <input type="number" id="edit_end_range" class="form-control"> --}}
                        </div>
                        <div class="form-group">
                            <label>Vehicle Year</label>
                            <input type="text" placeholder="Year" min="1900" max="2025" maxlength="4"
                                required class="form-control" id="year_check" name="year">
                        </div>
                        <div class="form-group">
    ...
    <script>
        $('#uploadForm').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('listed.listing.upload') }}",
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                ...
                success: function(response) {
                    if (response.success) {
                        $('#uploadStatus').html('<div class="alert alert-success">' + response.message +
                            '</div>');
                        loadzipcoderules();
                        setTimeout(function() {
                        ...
                        $('#uploadStatus').html('<div class="alert alert-danger">' + response.message +
                            '</div>');
                        $('#uploadBtn').prop('disabled', false);
                    }
                },
        ...
        function loadzipcoderules(page = 1) {
            currentPage = page;
            $.ajax({
                url: "{{ route('get.all.washington.listing') }}",
                type: "GET",
                success: function(response) {
                    if (response.success) {
                        let rows = '';
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="javascript:void(0);" class="editBtn" 
                                            ...
                                            data-dispatch_price="${item.price}"
                                            data-type="${item.type}"
        ...
        $('#editRecord').click(function() {
            let id = $('#edit_id').val();
            $.ajax({
                url: `/listed-listing/price/edit/${id}`,
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
        ...
        $(document).on('click', '.deleteBtn', function() {
            let id = $(this).data('id');
            $.ajax({
                url: `/listed-listing/price/delete/${id}`,
                type: "DELETE",
                data: {
                    _token: "{{ csrf_token() }}",
                },
                success: function(res) {
                    loadzipcoderules();
                    showSuccess('Listed Listing Price record deleted successfully');
                },
                error: function(xhr) {
                    showError(xhr.responseJSON?.message || 'Error updating record');
                }
